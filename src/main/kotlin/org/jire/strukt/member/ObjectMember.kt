...
class ObjectMember<T>(private val default: T) : Member() {

	private val current = ThreadLocal.withInitial { Long2ObjectArrayMap<T>() }

	operator fun getValue(ref: Strukt, prop: KProperty<*>): T = current.get()[ref.strukt.pointer]

	operator fun setValue(ref: Strukt, prop: KProperty<*>, value: T) {
